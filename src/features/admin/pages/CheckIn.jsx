...
function roleLabel(r) {
  return { captain:"Captain", abk:"ABK", gro:"GRO", staff:"Staff", other:"Other" }[r] ?? r;
}
...
function CrewResultCard({ data, onDismiss }) {
  const { crew, today_assignments, checkins_today, already_checked_in, just_checked_in, checked_in_at } = data;
  const color = ROLE_COLORS[crew.role] ?? "#888";

  return (
    <div className={`ci-result-card ${already_checked_in && !just_checked_in ? "ci-result-done" : "ci-result-active"}`}>
      {/* Header */}
      <div
        className="ci-scanned-passenger"
        style={{ borderLeft: `4px solid ${color}`, background: `${color}10` }}
      >
        <div className="ci-scanned-icon" style={{ background: `${color}20`, color }}>
        ...
        <div className="ci-scanned-info">
          ...
          <div className="ci-scanned-meta">
            <span className="ci-scanned-seat" style={{ background: `${color}15`, color }}>
              {roleLabel(crew.role)}
            </span>
      ...
      {checkins_today.length > 1 && (
        <div style={{ padding: "10px 20px", fontSize: 12, color: "var(--adm-text-muted)" }}>
          {checkins_today.length} kali scan hari ini ·{" "}
          Pertama: {fmtTime(checkins_today[checkins_today.length - 1]?.checked_in_at)}
        </div>
      )}

      {just_checked_in ? (
        <div className="ci-all-done" style={{ background: "#e8f5e9", color: "#2e7d32" }}>
...
function ResultCard({ result, scannedId, onCheckinAll, onDismiss, processing }) {
  ...
  return (
    <div className={`ci-result-card ${allDone ? "ci-result-done" : "ci-result-active"}`}>
      {scanned && (
        <div className={`ci-scanned-passenger ${isIn(scanned) ? "ci-scanned-in" : isCan(scanned) ? "ci-scanned-cancelled" : "ci-scanned-pending"}`}>
          <div className="ci-scanned-icon">
            {isIn(scanned) ? <UserCheck size={26} /> : isCan(scanned) ? <X size={26} /> : <ScanLine size={26} />}
          </div>
          <div className="ci-scanned-info">
            <div className="ci-scanned-label">
              {isIn(scanned) ? "✓ Sudah Check-In" : isCan(scanned) ? "✕ Tiket Dibatalkan" : "Tiket Ditemukan"}
            </div>
            <div className="ci-scanned-name">{scanned.passenger_name}</div>
            <div className="ci-scanned-meta">
              {scanned.seat_number && <span className="ci-scanned-seat">{scanned.seat_number}</span>}
              <span>{boat_name} · {direction === "RETURN" ? "Return" : "Departure"}</span>
            </div>
          </div>
      ...
      <div className="ci-group-header">
        <Users size={14} />
        <span>Grup: <strong>{group_name}</strong></span>
        <span className="ci-group-date">{fmtDate(trip_date)}</span>
      </div>

      <div className="ci-result-stats">
        <div className="ci-stat"><span className="ci-stat-val">{total}</span><span className="ci-stat-lbl">Total</span></div>
        <div className="ci-stat ci-stat-green"><span className="ci-stat-val">{checkedIn}</span><span className="ci-stat-lbl">Checked In</span></div>
        <div className="ci-stat ci-stat-orange"><span className="ci-stat-val">{pending}</span><span className="ci-stat-lbl">Pending</span></div>
        <div className="ci-stat ci-stat-red"><span className="ci-stat-val">{cancelled}</span><span className="ci-stat-lbl">Cancelled</span></div>
      </div>
      ...
      <div className="ci-result-tickets">
        {tickets.map((t) => {
          ...
          return (
            <div key={t.id} className={`ci-ticket-row ${isIn(t) ? "ci-ticket-in" : isCan(t) ? "ci-ticket-cancelled" : "ci-ticket-pending"} ${scannedRow ? "ci-ticket-scanned" : ""}`}>
              <span className="ci-ticket-dot">
                {isIn(t) ? <Check size={11} /> : isCan(t) ? <X size={11} /> : "·"}
              </span>
              <span className="ci-ticket-name">
                {t.passenger_name}
                {scannedRow && <span className="ci-ticket-this"> ← ini</span>}
              </span>
              {t.seat_number && <span className="ci-ticket-seat">{t.seat_number}</span>}
              <span className="ci-ticket-status">{isIn(t) ? "In" : isCan(t) ? "Batal" : "Pending"}</span>
            </div>
          );
        })}
      </div>
      ...
      {allDone ? (
        <div className="ci-all-done"><CheckCheck size={15} /> Semua penumpang sudah check-in</div>
      ) : (
        <button className="ci-checkin-btn" onClick={onCheckinAll} disabled={processing}>
          {processing
            ? <><RefreshCw size={14} className="ci-spin" /> Processing…</>
            : <><CheckCheck size={14} /> Check-In Semua yang Pending ({pending} orang)</>}
        </button>
      )}
    </div>
  );
...
export default function CheckIn() {
  ...
  const [result, setResult]                 = useState(null);       // passenger data
  const [crewResult, setCrewResult]         = useState(null);       // crew data
    ...
    try {
      ...
      if (data.type === "crew") {
        ...
        if (data.just_checked_in) {
          toast.success(`✓ ${data.crew.name} (${roleLabel(data.crew.role)}) checked in`);
        } else {
      ...
      // ── PASSENGER ──
      const sid = data.scanned_ticket_id ? Number(data.scanned_ticket_id) : null;
      setScannedId(sid);

      if (autoCheckin && sid) {
        const ticket = data.tickets.find((t) => Number(t.id) === sid);
        if (ticket && parseInt(ticket.checked_in) !== 1 && parseInt(ticket.cancelled) !== 1) {
          await api.post("/api/admin/manifest/checkin-bulk", { ticket_ids: [sid] }, { auth: true });
      ...
      dismissTimerRef.current = setTimeout(() => setResult(null), pending === 0 ? 5000 : 8000);
    } catch (e) {
      playBeep("error");
  ...
  const handleCheckinAll = async () => {
    ...
    const pendingIds = result.tickets
      .filter((t) => parseInt(t.checked_in) !== 1 && parseInt(t.cancelled) !== 1)
      .map((t) => t.id);
    if (!pendingIds.length) return;
    setProcessing(true);
    try {
      await api.post("/api/admin/manifest/checkin-bulk", { ticket_ids: pendingIds }, { auth: true });
      playBeep("success");
  ...
  const activeResult = crewResult ?? result;
      ...
      <div className="adm-page-header">
        <div>
          ...
          <p style={{ margin: 0, color: "var(--adm-text-muted)", fontSize: 14 }}>
            Scan QR boarding pass penumpang <strong>atau</strong> QR ID crew — otomatis check-in
          </p>
        </div>
      </div>
      ...
      <div className="ci-layout">
        ...
        <div className="ci-scanner-col">
          <div className="adm-card ci-scanner-card">
            ...
            <div className="ci-scanner-viewport">
              ...
              {!scannerActive && (
                <div className="ci-scanner-idle" onClick={() => setScannerActive(true)}>
            ...
            <div className="ci-scanner-status">
              <span className={`ci-status-dot ${scannerActive && scannerReady ? "ci-dot-green" : "ci-dot-gray"}`} />
              {scannerActive && scannerReady
                ? "Aktif — scan boarding pass atau QR ID crew"
                : scannerActive ? "Memulai kamera…" : "Kamera mati"}
            </div>

            <div className="ci-manual">
              <div className="ci-manual-label"><Search size={13} /> Input manual</div>
        ...
        <div className="ci-result-col">
          {crewResult ? (
            <CrewResultCard
              data={crewResult}
              onDismiss={() => { clearDismissTimer(); setCrewResult(null); }}
            />
          ) : result ? (
            <ResultCard
              result={result}
              ...
              onDismiss={() => { clearDismissTimer(); setResult(null); }}
              ...
              <div className="ci-log-list">
                {scanLog.map((entry) => (
                  <div key={entry.id} className="ci-log-row">
                    <div className="ci-log-icon" style={
                      entry.type === "crew"
                        ? { background: `${ROLE_COLORS[entry.role] ?? "#888"}20`, color: ROLE_COLORS[entry.role] ?? "#888" }
                        : undefined
                    }>
                      {entry.type === "crew" ? <Anchor size={12} /> : <Check size={12} />}
                    </div>
                    <div className="ci-log-info">
                      ...
                      <div className="ci-log-sub">
                        {entry.type === "crew"
                          ? (entry.alreadyIn ? "Sudah check-in sebelumnya" : "Check-in berhasil")
